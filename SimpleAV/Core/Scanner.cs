...
namespace TEAV.Core
{
    ...
    {
        private const long MAX_FILE_SIZE = 200 * 1024 * 1024; // 200 мб — больше не берём, всё равно не нужно

        // сканируем один файл — запускаем все три этапа по очереди
        public static ScanResult ScanFile(string filePath, HashSet<string> signatures)
        {
            ...
            try
            {
                ...
                // этап 1 — сигнатурное обнаружение, считаем хэши и ищем в базе
                ...
                if (match != null)
                {
                    ...
                    // нашли по сигнатуре — дальше анализировать смысла нет, это 100% малварь
                    return result;
                }
