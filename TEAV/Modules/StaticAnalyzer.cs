...
namespace TEAV.Modules
{
    ...
    public static class StaticAnalyzer
    {
        ...
        private static void ScanStrings(byte[] data, StaticResult result)
        {
            ...
            // сканируем с обоих выравниваний — .NET #US heap может начинаться с нечётного смещения
            string unicode = ExtractUnicode(data, 0) + "\n" + ExtractUnicode(data, 1);
        ...
        // извлекаем utf-16le строки начиная с заданного смещения (0 или 1)
        // два прохода нужны потому что .NET хранит строки в #US heap и выравнивание не гарантировано
        private static string ExtractUnicode(byte[] data, int startOffset)
        {
