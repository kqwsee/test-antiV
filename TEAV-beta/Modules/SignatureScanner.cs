...
namespace TEAV.Modules
{
    ...
    public static class SignatureScanner
    {
        ...
        {
            if (db.Contains(md5))   return new SignatureMatch { Algorithm = "MD5",    Hash = md5 };
            if (db.Contains(sha1))  return new SignatureMatch { Algorithm = "SHA1",   Hash = sha1 };
            if (db.Contains(sha256)) return new SignatureMatch { Algorithm = "SHA256", Hash = sha256 };
            return null;
        }
